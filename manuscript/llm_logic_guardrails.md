 ...
 *
 * Validates investment recommendations generated by LLMs using symbolic rules.
...
% 2. If client is senior (> 65), high-risk assets (stocks + crypto) must be <= 30%
check_policy(Dict, Error) :-
    ...
    format(string(Error), "Senior client (age ~w) has ~w% in high-risk assets (max: 30%)", [Age, HighRiskAllocation]).

% 3. If risk tolerance is 'low', crypto allocation must be 0%
check_policy(Dict, "Low risk tolerance portfolio cannot contain speculative crypto assets") :-
    Risk = Dict.get(risk_tolerance),
    Risk == "low",
    get_allocations(Dict, _, _, Crypto, _),
    Crypto > 0.

% 4. If risk tolerance is 'low', conservative assets (bonds + cash) must be >= 50%
check_policy(Dict, Error) :-
    Risk = Dict.get(risk_tolerance),
    Risk == "low",
    ...
    Conservative < 50,
    format(string(Error), "Low risk tolerance requires at least 50% in conservative assets (currently ~w%)", [Conservative]).
